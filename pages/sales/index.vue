    ...
    <section class="flex-1 bg-white rounded-lg shadow border border-gray-100 flex flex-col min-h-0">
      <div class="flex-1 overflow-y-auto pb-16">
        <table class="w-full">
          ...
          <thead class="bg-gray-50 sticky top-0 z-10">
            <tr>
              ...
              <th class="px-4 py-3 text-center text-sm font-semibold text-gray-700 border-b w-24">Qté</th>
              <th class="px-4 py-3 text-center text-sm font-semibold text-gray-700 border-b w-24">Valeur</th>
          ...
          <tbody>
            <template v-for="product in filteredProducts" :key="product.id">
              ...
              >
                ...
                >
                  <div class="flex flex-col">
                    <span 
                      :class="['text-sm font-medium', getQuantityBySize(product, size) > 0 ? 'text-gray-900' : 'text-gray-300']"
                    >
                      {{ getQuantityBySize(product, size) }}
                    </span>
                    <span 
                      v-if="getQuantityBySize(product, size) > 0"
                      class="text-xs text-green-600"
                    >
                      {{ formatPrice(getValueBySize(product, size)) }}
                    </span>
                  </div>
                </td>
                <td class="px-4 py-3 text-center border-b">
                  <span class="text-sm font-bold text-primary">
                    {{ getTotalQuantityForProduct(product) }}
                  </span>
                </td>
                <td class="px-4 py-3 text-center border-b">
                  <span class="text-sm font-bold text-green-600">
                    {{ formatPrice(getTotalValueForProduct(product)) }}
                  </span>
                </td>
                <td class="px-4 py-3 text-center border-b">
                  <button 
              ...
              <template v-if="isProductExpanded(product.id)">
                <tr 
                ...
                >
                  ...
                  >
                    <div class="flex flex-col">
                      <span 
                        :class="['text-xs', getQuantityBySizeAndColor(product, size, color) > 0 ? 'text-gray-700' : 'text-gray-300']"
                      >
                        {{ getQuantityBySizeAndColor(product, size, color) }}
                      </span>
                      <span 
                        v-if="getQuantityBySizeAndColor(product, size, color) > 0"
                        class="text-xs text-green-600"
                      >
                        {{ formatPrice(getValueBySizeAndColor(product, size, color)) }}
                      </span>
                    </div>
                  </td>
                  <td class="px-4 py-2 text-center font-semibold">
                    {{ getTotalQuantityByColor(product, color) }}
                  </td>
                  <td class="px-4 py-2 text-center font-semibold text-green-600">
                    {{ formatPrice(getTotalValueByColor(product, color)) }}
                  </td>
        ...
        <div v-if="filteredProducts.length === 0" class="text-center py-12 text-gray-500">
          ...
          <div class="text-lg">Aucune vente trouvée</div>
          <div class="text-sm mt-1">Essayez de modifier vos filtres ou la période</div>
        </div>
      </div>
        ...
        <div class="flex items-center justify-end">
          ...
          <template v-for="size in sizeOrder" :key="size">
            <div :class="['w-16 px-3 py-3 text-center font-bold', filterSize === size ? 'bg-yellow-200' : '']">
              <div class="flex flex-col">
                <span class="text-lg">{{ getTotalQuantityBySize(size) }}</span>
                <span v-if="getTotalQuantityBySize(size) > 0" class="text-xs text-green-700">
                  {{ formatPrice(getTotalValueBySize(size)) }}
                </span>
              </div>
            </div>
          </template>
          <div class="w-24 px-4 py-3 text-center text-xl text-primary font-bold">{{ totalQuantity }}</div>
          <div class="w-24 px-4 py-3 text-center text-xl text-green-700 font-bold">{{ formatPrice(totalValue) }}</div>
...
import { useSalesStore, type ProductType, type Size, type Color } from '@/stores/salesStore';
...
// Dates par défaut: du 1er jour du mois courant à aujourd'hui
...
// ---- Constants ------------------------------------------------
const { allTypes, allSizes, allColors, sizeOrder, getQuantityBySize, getValueBySize, getQuantityBySizeAndColor, getValueBySizeAndColor, getColorsForProduct, getTotalQuantityForProduct, getTotalValueForProduct, getTotalQuantityByColor, getTotalValueByColor } = salesStore;
...
// Récupérer les produits agrégés selon la période
const aggregatedProducts = computed(() => {
  // Use the pre-aggregated data from the store
  return salesStore.aggregatedSales;
});

const filteredProducts = computed(() => {
  let result = aggregatedProducts.value;
...
const totalQuantity = computed(() => {
  return filteredProducts.value.reduce((sum, p) => sum + getTotalQuantityForProduct(p), 0);
});

const totalValue = computed(() => {
  return filteredProducts.value.reduce((sum, p) => sum + getTotalValueForProduct(p), 0);
});

// Totaux par taille
const getTotalQuantityBySize = (size: Size): number => {
  return filteredProducts.value.reduce((sum, p) => sum + getQuantityBySize(p, size), 0);
};

const getTotalValueBySize = (size: Size): number => {
  return filteredProducts.value.reduce((sum, p) => sum + getValueBySize(p, size), 0);
};
...
const getFilteredColorsForProduct = (product: { variations: { color: Color }[] }) => {
